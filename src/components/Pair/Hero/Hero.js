    ...
    return (
      <I18n ns="translations">
        {t => (
          <>
            <SEO
              title={t('PerExchangePairHEAD.title', {
                receiveCoinFullName: baseCurrency && baseCurrency.name && capitalize(baseCurrency.name),
                depositCoinSymbol: this.props.selectedCoin.deposit.toUpperCase(),
                depositCoinFullName: quoteCurrency && quoteCurrency.name && capitalize(quoteCurrency.name),
                receiveCoinSymbol: this.props.selectedCoin.receive.toUpperCase(),
              })}
              description={t('PerExchangePairHEAD.meta_description', {
                receiveCoinFullName: baseCurrency && baseCurrency.name && capitalize(baseCurrency.name),
                depositCoinSymbol: this.props.selectedCoin.deposit.toUpperCase(),
                depositCoinFullName: quoteCurrency && quoteCurrency.name && capitalize(quoteCurrency.name),
                receiveCoinSymbol: this.props.selectedCoin.receive.toUpperCase(),
