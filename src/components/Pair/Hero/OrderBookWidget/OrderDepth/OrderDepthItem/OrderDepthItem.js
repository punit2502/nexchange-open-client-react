...
class OrderDepthItem extends PureComponent {
  ...
  getMyOrderSize(limit_rate) {
    ...
    const myMatchingOrders = _.filter(
      myOrders,
      order =>
    ...
    return (
      <I18n ns="translations">
        {t => (
          ...
          <div
            className={`${styles.container} ${styles[this.props.side]}`}
            style={{ background: sizeBackground }}
            data-test={`${this.props.side}`}
          >
            <a
              onClick={() => this.setOrderBookQuantity(item.size)}
              className={`clickable ${myOrderSize ? 'bold' : ''}`}
              /* eslint max-len: ["error", { "code": 200 }] */

              title={
                myOrderSize
                  ? `${this.props.side} ${myOrderSize} ${this.props.selectedCoin.receive} @ ${item.rate} ${this.props.selectedCoin.receive}${this.props.selectedCoin.deposit}`
                  : null
              }
            >
              {`${item.size.toFixed(9)}`}
            ...
            <a
              onClick={() => this.setOrderBookLimitRate(item.rate)}
              className={`clickable ${myOrderSize ? 'bold' : ''}`}
              /* eslint max-len: ["error", { "code": 200 }] */
...
export default connect(mapStateToProps, mapDispatchToProps)(OrderDepthItem);
